...
class secBootUiSettingsFlexibleUserKeys(advSettingsWin_FlexibleUserKeys.advSettingsWin_FlexibleUserKeys):
    ...
    def __init__(self, parent):
        ...
        self.engine0FacStart = [None] * RTyyyy_uidef.kMaxFacRegionCount
        self.engine0FacLength = [None] * RTyyyy_uidef.kMaxFacRegionCount
        self.engine1FacStart = [None] * RTyyyy_uidef.kMaxFacRegionCount
        self.engine1FacLength = [None] * RTyyyy_uidef.kMaxFacRegionCount
    ...
    def _getEngineRange( self, engineIndex=0, facIndex=0 ):
        if engineIndex == 0:
            if facIndex == 0:
                ...
                if validateStatus:
                    if self.engine0FacStart[0] < RTyyyy_rundef.kBootDeviceMemBase_FlexspiNor + RTyyyy_gendef.kIvtOffset_NOR:
                        self.popupMsgBox('Engine 0 Protected region 0 start address shouldn\'t less than 0x%x' %(RTyyyy_rundef.kBootDeviceMemBase_FlexspiNor + RTyyyy_gendef.kIvtOffset_NOR))
                        return False
                else:
                    return False
                validateStatus, self.engine0FacLength[0] = self._validateEngineRange(self.m_textCtrl_engine0Fac0Length.GetLineText(0))
                if validateStatus:
                    if self.engine0FacLength[0] % RTyyyy_gendef.kSecFacRegionAlignedUnit != 0:
                        self.popupMsgBox('Engine 0 Protected region 0 length should be aligned with %dKB' %(RTyyyy_gendef.kSecFacRegionAlignedUnit / 0x400))
                        return False
                else:
                    return False
                ...
                if validateStatus:
                    if self.engine0FacLength[1] % RTyyyy_gendef.kSecFacRegionAlignedUnit != 0:
                        self.popupMsgBox('Engine 0 Protected region 1 length should be aligned with %dKB' %(RTyyyy_gendef.kSecFacRegionAlignedUnit / 0x400))
                        return False
                else:
                    return False
                ...
                if validateStatus:
                    if self.engine0FacLength[2] % RTyyyy_gendef.kSecFacRegionAlignedUnit != 0:
                        self.popupMsgBox('Engine 0 Protected region 2 length should be aligned with %dKB' %(RTyyyy_gendef.kSecFacRegionAlignedUnit / 0x400))
                        return False
                else:
                    return False
            ...
            if facIndex == 0:
                ...
                if validateStatus:
                    if self.userKeyCtrlDict['engine_sel'] == RTyyyy_uidef.kUserEngineSel_BothEngines:
                        ...
                        if self.engine1FacStart[0] < RTyyyy_rundef.kBootDeviceMemBase_FlexspiNor + RTyyyy_gendef.kIvtOffset_NOR:
                            self.popupMsgBox('Engine 1 Protected region 0 start address shouldn\'t less than 0x%x' %(RTyyyy_rundef.kBootDeviceMemBase_FlexspiNor + RTyyyy_gendef.kIvtOffset_NOR))
                            return False
                else:
                    return False
                validateStatus, self.engine1FacLength[0] = self._validateEngineRange(self.m_textCtrl_engine1Fac0Length.GetLineText(0))
                if validateStatus:
                    if self.engine1FacLength[0] % RTyyyy_gendef.kSecFacRegionAlignedUnit != 0:
                        self.popupMsgBox('Engine 1 Protected region 0 length should be aligned with %dKB' %(RTyyyy_gendef.kSecFacRegionAlignedUnit / 0x400))
                        return False
                else:
                    return False
                ...
                if validateStatus:
                    if self.engine1FacLength[1] % RTyyyy_gendef.kSecFacRegionAlignedUnit != 0:
                        self.popupMsgBox('Engine 1 Protected region 1 length should be aligned with %dKB' %(RTyyyy_gendef.kSecFacRegionAlignedUnit / 0x400))
                        return False
                else:
                    return False
                ...
                if validateStatus:
                    if self.engine1FacLength[2] % RTyyyy_gendef.kSecFacRegionAlignedUnit != 0:
                        self.popupMsgBox('Engine 1 Protected region 2 length should be aligned with %dKB' %(RTyyyy_gendef.kSecFacRegionAlignedUnit / 0x400))
                        return False
                else:
                    return False
    ...
    def callbackChangeEngine0FacCnt( self, event ):
        if self.userKeyCtrlDict['engine_sel'] == RTyyyy_uidef.kUserEngineSel_Engine0:
            ...
            if region0FacCnt + self.userKeyCtrlDict['engine1_fac_cnt'] > RTyyyy_uidef.kMaxFacRegionCount:
                self.m_choice_engine0FacCnt.SetSelection(self.userKeyCtrlDict['engine0_fac_cnt'] - 1)
                self.popupMsgBox('The sum of Protected Region count of Engine0 and Engine1 must be no more than ' + str(RTyyyy_uidef.kMaxFacRegionCount))
            else:
                self._getFacCount(0)
                self._updateFacRangeInfoField(0)
        else:
    ...
    def callbackChangeEngine1FacCnt( self, event ):
        if self.userKeyCtrlDict['engine_sel'] == RTyyyy_uidef.kUserEngineSel_Engine1:
            ...
            if region1FacCnt + self.userKeyCtrlDict['engine0_fac_cnt'] > RTyyyy_uidef.kMaxFacRegionCount:
                self.m_choice_engine1FacCnt.SetSelection(self.userKeyCtrlDict['engine1_fac_cnt'] - 1)
                self.popupMsgBox('The sum of Protected Region count of Engine0 and Engine1 must be no more than ' + str(RTyyyy_uidef.kMaxFacRegionCount))
            else:
                self._getFacCount(1)
